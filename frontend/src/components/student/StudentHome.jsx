...
export default function StudentHome() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // Fetch stats
        const statsResponse = await axios.post("http://localhost:4000/api/team/dash_data", {
          id: fetchedStudentId,
        });
  ...
  const handleFileUpload = async (registrationId, file) => {
    ...
    try {
      ...
      const uploadResponse = await axios.post("http://localhost:4000/api/team/upload_cert", formData, {
  ...
  const handleStatusUpdate = async (registrationId, newStatus) => {
    ...
    try {
      const statusResponse = await axios.put("http://localhost:4000/api/team/update-status", {
